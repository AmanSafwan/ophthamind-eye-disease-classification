...
def base_preprocess(image: Image.Image):
    """
    Standard preprocessing pipeline (NO model-specific transform here)
    """
...
def predict_model(model, x):
    ...
    pred = np.nan_to_num(pred)  # safety
    return pred

# =====================================================
# ENSEMBLE STRATEGY (WEIGHTED - MORE STABLE)
# =====================================================
def ensemble_predictions(cnn_pred, vgg_pred, resnet_pred):
    # Weighted ensemble (IMPORTANT for stability)
    return (
        cnn_pred * 0.30 +
        vgg_pred * 0.35 +
        resnet_pred * 0.35
...
# =====================================================
# AGREEMENT SCORE (MORE ACCURATE VERSION)
# =====================================================
...
def _repair_stdio_for_background_launch():
    """PHP `start /B` on Windows can leave stdout in a broken state; Flask/click then crash on banner."""
    import os
    import sys
